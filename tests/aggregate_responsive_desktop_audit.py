...
expected=['tablet_landscape_1024x768','laptop_1366x768','desktop_fullhd_1920x1080','desktop_ultrawide_2560x1080']
reports=[]; errors=[]
for name in expected:
    path=ROOT/'tests'/f'responsive-desktop-results-{name}.json'
    if not path.exists(): errors.append(f'missing report: {path.name}'); continue
    try: report=json.loads(path.read_text(encoding='utf-8'))
    except Exception as exc: errors.append(f'invalid report {path.name}: {exc}'); continue
...
(ROOT/'tests/responsive-desktop-results.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
...
(ROOT/'tests/responsive-desktop-output.txt').write_text('\n'.join(lines)+'\n',encoding='utf-8')
print('\n'.join(lines))
